...
const BlogUpdate2: React.FC<IParams> = ({ params }) => {
  ...
  const { data, isLoading, error, refetch } = useDetailBlogQuery(params.slug, {
    skip: !shouldRefetch, // Skip initial fetch if shouldRefetch is false
  });
  ...
  useEffect(() => {
    if (params.slug) {
      setShouldRefetch(true); // Trigger refetch when slug changes
    }
  }, [params.slug]);
  ...
  const handleUpdate = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Yönlendirme işlemi
      const updatedSlug = response.data?.data?.slug;
      if (updatedSlug) {
        setShouldRefetch(true); // Trigger refetch after redirect
        await router.push(`/blog/myblog/${updatedSlug}`);
      }
    } catch (error) {
